...
def train(args, model, tokenizer, train_dataset, one_epoch=False, do_print=False):
    ...
    for epoch in range(args.epochs):
        ...
        if do_print:
            print(f"Epoch {epoch}, Accuracy: {char_accuracy_list[0]}, Loss: {avg_loss}")
...
def get_ident_name(args):
    ...
    # Depth tests
    return "run_%s_%s_w%d_d%d_nh%d_sd%d" % (dashed_task_name, args.model, args.window, args.hidden_size, \
