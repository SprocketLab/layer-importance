...
from .gdn import GatedLinearLayer
...
import warnings
warnings.filterwarnings("ignore", message="ShortConvolution is crucial")
...
class HybridCausalLMOutput(ModelOutput):
    ...
    cache_params: Optional[MambaCache] = None
    hidden_states: Optional[tuple[torch.FloatTensor]] = None
    gdn_states: Optional[tuple[torch.FloatTensor]] = None
    past_key_values: Optional[torch.FloatTensor] = None
    last_hidden_state: Optional[torch.FloatTensor] = None
    attentions: Optional[torch.FloatTensor] = None
...
class HybridModel(HybridPreTrainedModel):
    def __init__(self, config):
        ...
        layer_types = [MambaBlock, GPTNeoXLayer, GatedLinearLayer]
        ...
        for idx, layer in enumerate(config.layers):
            if layer == "SSM":
                ...
                modules.append(GatedLinearLayer(config, idx))
            else:
    ...
    def get_head_mask(self, head_mask: Optional[torch.Tensor], num_hidden_layers: int, is_attention_chunked: bool = False) -> torch.Tensor:
        ...
        head_mask = head_mask.view(-1, 1)
        if is_attention_chunked:
            head_mask = head_mask.expand(-1, num_hidden_layers)
        return head_mask
    ...
    def forward(
        ...
        output_gdn_states: Optional[bool] = None,
        ...
        all_gdn_states = () if output_gdn_states else None
        for i, (layer, layer_past) in enumerate(zip(self.layers, past_key_values)):
            ...
            if isinstance(layer, GPTNeoXLayer):
                ...
                if output_gdn_states:
                    all_gdn_states = all_gdn_states + (None,)
            ...
            elif isinstance(layer, GatedLinearLayer):
                outputs = layer(
                    input_embeds=hidden_states,
                    output_gdn_states=output_gdn_states
                )
                hidden_states = outputs[0]
                ...
                if output_attentions:
                    all_attentions = all_attentions + (None,)
                if output_gdn_states:
                    all_gdn_states = all_gdn_states + (outputs[1],)
