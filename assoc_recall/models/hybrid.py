...
from .delta_product import DeltaProductBlock, HAS_FLA
...
class HybridModel(HybridPreTrainedModel):
    def __init__(self, config):
        ...
        for idx, layer in enumerate(config.layers):
            if layer == "SSM":
                ...
                if not HAS_FLA:
                    raise ImportError("flash-linear-attention required for DP layers")
                modules.append(DeltaProductBlock(config, layer_idx=idx))
            else:
    ...
    def forward(
        ...
        for i, (layer, layer_past) in enumerate(zip(self.layers, past_key_values)):
            ...
            elif isinstance(layer, DeltaProductBlock):
